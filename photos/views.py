...
from photos.forms import AddImageForm, UpdateImageForm
...
def home(request):
    ...
    context = {'photos': images, 'images_p': images_p}
...
def add_image(request):
    ...
    context = {'form': form}
    return render(request, "photos/forms/add_image.html", context)


def delete_image(request, id_image):
    image = Image.objects.get(pk=id_image)
    image.delete()
    messages.success(request, "The photos has been correctly added")
...
def update_image(request, id_image):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "The photos has been correctly updated")
    form = UpdateImageForm(instance=image)

    context = {'form': form}
    return render(request, "photos/forms/edit_image.html", context)
